<body class="bg-light">
	<div class="container">
		...
		if (isset($_POST["register"])) {
			if (registrasi($_POST)>0) {
				...
				echo "<meta http-equiv='refresh' content='2;url=index.php?page=login-tutor'>";
			}
		}else{
  ...
  //cek username sudah ada atau belum
			$result = mysqli_query($conn, "SELECT username_tutor FROM tabel_tutor WHERE username_tutor = '$username'");
			...
			// cek email 
			$hasil = mysqli_query($conn, "SELECT email_tutor FROM tabel_tutor WHERE email_tutor = '$email'");
  ...
  //enkripsi password
   ...
   //menambahkan ke database
			mysqli_query($conn, "INSERT INTO tabel_tutor (nama_depan_tutor, username_tutor, email_tutor, password_tutor) VALUES ('$nama', '$username', '$email', '$password')");
